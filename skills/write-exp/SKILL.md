...
- For heap/tcache work, prefer `Mypwn` helpers over ad-hoc formulas: `safe_linking_encrypt`, `safe_linking_decrypt`, `safe_linking_demangle`, `tcache_idx`, `tcache_layout`, and especially `write_tcache({0x18: target})` for quick `tcache_perthread_struct` payloads. Drop to `TcachePerThreadStruct` or `edit_tcache_perthread` only when precise low-level patching is needed. Pass `glibc_version` when layout or safe-linking behavior matters, and override `ptr_size`, `bins`, `count_size`, or `filler` only for custom/backported allocators.
...
- When editing `tcache_perthread_struct`, start with `write_tcache({0x18: stack_anchor_target}, data=raw, glibc_version=...)`; dict keys are malloc request sizes by default, so use `key="chunk"` or `key="idx"` only when needed. Use `filler=` for the unmodified bytes/count area, and remember glibc 2.42+ uses `num_slots` with 76 bins.
